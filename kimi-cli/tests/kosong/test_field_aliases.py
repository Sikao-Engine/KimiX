...
def _discover_params_classes(base_dir: Path, package_prefix: str) -> list[type[BaseModel]]:
    ...
    for _finder, module_name, is_pkg in pkgutil.iter_modules(
        ...
        for _name, obj in inspect.getmembers(module, inspect.isclass):
            if obj is BaseModel:
                continue
            if issubclass(obj, BaseModel) and (_name == "Params" or _name.endswith("Params")):
                classes.append(obj)
    return classes
...
def test_repair_dict_parses_json_string_with_nested_model_inside_list_items() -> None:
    ...
    data = {"questions": '[{"content": "q1", "choices": [{"label": "a"}]}]'}
...
def test_callable_tool2_accepts_json_encoded_nested_model() -> None:
    ...
    tool = TestTool()
    result = asyncio.run(tool.call({"name": "foo", "inner": '{"value": "bar"}'}))
