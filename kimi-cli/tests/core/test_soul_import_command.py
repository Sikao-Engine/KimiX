...
async def test_import_directory_path_reports_clear_error(tmp_path: Path, monkeypatch) -> None:
    ...
    soul = Mock()
    await soul_slash.cmd_import(soul, str(target_dir))  # type: ignore[reportGeneralTypeIssues]
...
async def test_export_writes_file_and_sends_wire(tmp_path: Path, monkeypatch) -> None:
    ...
    await soul_slash.cmd_export(soul, str(output))  # type: ignore[reportGeneralTypeIssues]
...
async def test_import_file_sends_wire_markers(tmp_path: Path, monkeypatch) -> None:
    ...
    await soul_slash.cmd_import(soul, str(source))  # type: ignore[reportGeneralTypeIssues]
...
async def test_import_env_file_sends_warning(tmp_path: Path, monkeypatch) -> None:
    ...
    await soul_slash.cmd_import(soul, str(env_file))  # type: ignore[reportGeneralTypeIssues]
