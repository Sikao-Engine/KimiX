...
def _convert_error(error: Exception) -> ChatProviderError:
    ...
    if isinstance(error, genai_errors.ClientError):
        ...
        if status_code == 401:
            return APIStatusError(401, f"Authentication failed: {error}", headers=headers)
        elif status_code == 403:
            return APIStatusError(403, f"Permission denied: {error}", headers=headers)
        elif status_code == 429:
            return APIStatusError(429, f"Rate limit exceeded: {error}", headers=headers)
        ...
        # Generic API errors
        status_code = getattr(error, "code", 500)
        return APIStatusError(status_code, str(error), headers=_response_headers(error))
