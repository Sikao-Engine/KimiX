...
class OpenAILegacy(OpenAICompatibleProviderMixin):
    ...
    async def generate(
        ...
        if self._reasoning_key is not None:
            ...
            if self._openai_settings.get("chat_template_kwargs", True):
                extra_body["chat_template_kwargs"] = {"reasoning_effort": extra_body_level}
    ...
    def _convert_message(self, message: Message) -> ChatCompletionMessageParam:
        ...
        # recursive deep copy.
        needs_mutation = (message.role == "assistant" and message.tool_calls) or (
            message.role == "tool" and self._tool_message_conversion == "extract_text"
        )
        ...
        if message.role == "tool" and (
            error_msg := check_tool_call_id(message.tool_call_id, message.extract_text(sep="\n"))
        ):
            return cast(
                ChatCompletionMessageParam,
