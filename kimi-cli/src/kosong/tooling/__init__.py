...
TOOL_NAME_REDIRECTS: dict[str, str] = {
    ...
    "Read": "read",
    # Legacy class-name redirects (old tool names -> report canonical names).
    "ReadFile": "read",
    "WriteFile": "write",
    "EditFile": "edit",
    "Glob": "glob",
...
def _format_pydantic_validation_error(
    ...
    lines: list[str] = []
    lines.append(
        f"Invalid arguments for tool `{tool_name}` — {len(errors_list)} validation error(s):"
    )
    lines.append("")
    ...
    if params_schema is not None:
        ...
        has_structural_error = any(e.get("type", "") in structural_error_types for e in errors_list)
