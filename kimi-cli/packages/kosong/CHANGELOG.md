...
- OpenAI Legacy: Fix `reasoning_content` validation errors on Moonshot/Kimi-compatible backends by passing the field back on every assistant message when `reasoning_key` is configured (empty string for assistant turns without reasoning). DeepSeek/Moonshot-compatible backends require the field to be present on all assistant turns once thinking/tool-call chains have been used.
...
- OpenAI/Kimi: Add an opt-in debug helper (`maybe_log_reasoning_content_error`) for the Moonshot/Kimi 400 "reasoning_content in the thinking mode must be passed back to the API" error. The helper can log request context (provider, model, messages, generation kwargs, and error details) to `./error.log`, but it is disabled by default; the call sites in OpenAI Legacy, OpenAI Responses, and Kimi providers are commented out.
- Kimi: When thinking mode is enabled (or the history already contains reasoning content), pass back `reasoning_content` on every assistant message (empty string for assistant turns without reasoning). This aligns the native Moonshot provider with the DeepSeek/Moonshot requirement that the field be present on all assistant turns in thinking-mode conversations.
- OpenAI Legacy: Auto-detect Moonshot models (`kimi-*`) and Moonshot base URLs and disable the `thinking`/`reasoning`/`chat_template_kwargs` auto-generated `extra_body` keys for them. Moonshot uses the standard OpenAI `reasoning_effort`/`reasoning_content` wire format and the provider-specific extra_body keys can confuse its validator.
