...
class ProcessTask:
    ...
    async def _run_process_bg(self, q: queue.Queue[str]) -> bool:
        ...
        try:
            ...
            # On Windows, put the child in its own process group so that
            # console control signals (Ctrl+C/Ctrl+Break) generated by or
            # sent to the child are not broadcast back to the parent Python
            # process. This prevents PowerShell and other console apps from
            # raising KeyboardInterrupt in the current process.
            if os.name == "nt":
